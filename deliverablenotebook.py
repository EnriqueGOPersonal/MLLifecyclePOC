...
print(data["train"]["transactional_data_gr"].columns.values)
...
# It is necessary to specifically handle missing values where client has no transactions.
...
g = sns.PairGrid(eda_df[num_cols[0:6] +["defaulted_loan"]], hue = "defaulted_loan", height = 10)
g.map_diag(plt.hist)
...
# Choosing a model of the goals of implementation. In this situation, the tradeoff between identifying more defaulted loans at the cost of misclassification can be analyzed with a specific tool called a roc curve.  When the model predicts a class label, a probability threshold is used to make the decision. This threshold is set by default at 50% so that observations with more than a 50% chance of membership belong to one class and vice-versa.
